"""Geração de relatórios PDF — dashboard completo OU gráfico individual.

Layout:
    - Cabeçalho institucional + data/hora
    - Bloco(s) de gráfico em alta definição (matplotlib 200dpi)
    - Insight clínico
    - Rodapé LGPD com paginação
"""
...
# Paleta HSL convertida para hex (alinhada ao frontend)
PALETTE_HEX = [
    "#1f8be0",  # primary
    "#1ec1a3",  # accent
    "#f59e0b",  # warning
...
# ---------------------------------------------------------------------------
# Renderização de gráfico (matplotlib HD)
# ---------------------------------------------------------------------------
...
def _styles():
    ...
    return {
        ...
        "sub": ParagraphStyle("Sub", parent=base["Normal"], fontSize=9.5,
        ...
        "insight": ParagraphStyle("Insight", parent=base["BodyText"],
                                  fontSize=9.5,
...
def _new_doc(buf: io.BytesIO, titulo: str) -> SimpleDocTemplate:
...
def _chart_block(c: dict, st: dict, idx: int = 0) -> list:
    """Bloco reutilizável: título + imagem + insight."""
    cd = c.get("chartData") or {}
    titulo = c.get("titulo") or cd.get("title") or f"Gráfico {idx + 1}"
    block: list = [Paragraph(titulo, st["h2"])]
    ...
    if img_buf is not None:
        ...
        block.append(Paragraph("(Gráfico indisponível)", st["body"]))

    insight_text = c.get("suggested_insight") or c.get("sugestao")
    if insight_text:
        block.append(Paragraph(f"<b>Insight:</b> {insight_text}", st["insight"]))
...
def gerar_pdf_dashboard(
    ...
    story: list = [
        Paragraph(titulo_relatorio, st["h1"]),
        Paragraph(f"Gerado em {datetime.now().strftime('%d/%m/%Y às %H:%M')}",
                  st["sub"]),
    ]

    if analise_texto:
        story.append(Paragraph("Análise", st["h2"]))
        story.append(Paragraph(analise_texto.replace("\n", "<br/>"), st["body"]))

    charts = list(charts)
    if not charts:
        story.append(Paragraph("Nenhum gráfico fixado no dashboard no momento.",
                               st["body"]))
    else:
        story.append(Paragraph("Gráficos fixados", st["h2"]))
        for idx, c in enumerate(charts):
            story.extend(_chart_block(c, st, idx))
...
def gerar_pdf_chart(
    ...
    suggested_insight: str | None = None,
    ...
    story: list = [
        Paragraph(titulo_render, st["h1"]),
        Paragraph(f"Gerado em {datetime.now().strftime('%d/%m/%Y às %H:%M')}",
                  st["sub"]),
    ]
    if analise:
        story.append(Paragraph("Análise", st["h2"]))
        story.append(Paragraph(analise.replace("\n", "<br/>"), st["body"]))

    story.extend(_chart_block(
        {"titulo": titulo_render, "chartData": chart_data,
